...
# 函数build_cnn(arch, normalization, activation, padding, pooling, init)根据给定的架构字符串
# 构建一个卷积神经网络。架构字符串是一个由层规范字符串组成的列表或逗号分隔的字符串。
# 函数返回一个包含卷积神经网络的nn.Sequential对象和输出通道数。
...
# 函数build_mlp(dim_list, activation, batch_norm, dropout, final_nonlinearity)根据给定的维度列表
# dim_list：一个整数列表，表示MLP各层的输入和输出维度。
# 构建一个多层感知机。维度列表是一个整数列表，表示每个隐藏层的输入和输出维度。
# 函数返回一个包含多层感知机的nn.Sequential对象。
# final_nonlinearity：是否在最后一层添加非线性激活函数。
